...
Feature: Put
  ...
  Scenario: User success put first name data
    ...
    When User sends a PUT request to update the user with id "60d0fe4f5311236168a109ce" and the following data:
  ...
  Scenario: User success put email data
    Given The user API is available
    When User sends a PUT request to update the user with id "60d0fe4f5311236168a109ce" and email data:
  ...
  Scenario: User put with wrong id
    Given The user API is available
    When User sends a PUT request to update the user with id "Invalid12345" and the following data:
      | firstName | UpdatedName |
    Then The response status put should be 400
    And The response body should contain error message "BODY_NOT_VALID"